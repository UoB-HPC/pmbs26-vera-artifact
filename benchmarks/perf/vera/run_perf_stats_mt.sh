#!/usr/bin/env bash
# Same collection as run_perf_stats.sh, but with SMT enabled: every miniapp
# and every NAS kernel (Class B and Class D) runs using all logical CPUs
# (2 threads per physical core) instead of 1 thread per physical core.
# Results are written to a separate directory so they don't overwrite the
# non-SMT run.
#
# ==== PORTING TO ANOTHER MACHINE/ARCHITECTURE ====
# - Physical core count and total logical CPU count are auto-detected
#   below; nothing to change there.
...
RESULTS_DIR=$HOME/joseph/results/perf-mt
...
preflight_check
echo "Physical cores detected: $CORES, using $THREADS SMT threads (cpu range $CPU_RANGE, threads-per-core $THREADS_PER_CORE)"

for class in "${NAS_CLASSES[@]}"; do
	echo "-----NAS Class $class (MT)-----"
