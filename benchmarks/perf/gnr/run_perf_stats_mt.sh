...
# misses, TLB misses, cycles, instructions, etc.) for one full-machine run
# WITH SMT enabled (all logical CPUs / hyperthreads in use, 2 threads per
# physical core) of every miniapp, plus every NAS kernel at Classes B and D.
# This is a separate, one-shot profiling pass -- it does not do any
# core-count scaling; see scripts/nas/run_nas_scaling.sh and the per-app
# run_*_n-core.sh scripts for that.
#
# See run_perf_stats.sh for the non-SMT (1 thread per physical core)
# counterpart.
#
# ==== PORTING TO ANOTHER MACHINE/ARCHITECTURE ====
# - Physical core and logical thread counts are auto-detected below;
...
RESULTS_DIR=$HOME/gnr-results/perf-mt
...
THREAD_END=$((THREADS - 1))
CPU_RANGE="0-$THREAD_END"
...
record_perf_events() {
	local name=$1
	awk -v app="$name" -v cores="$CORES" -v threads="$THREADS" '
...
record_metric() {
	...
	printf '%s,%s,%s,%s,%s\n' "$name" "$CORES" "$THREADS" "$metric" "$value" >> "$METRICS_CSV"
	printf '  %s: %s = %s\n' "$name" "$metric" "$value"
}

preflight_check
echo "Physical cores detected: $CORES, logical threads: $THREADS (cpu range $CPU_RANGE, threads-per-core $THREADS_PER_CORE)"

for cls in "${NAS_CLASSES[@]}"; do
	echo "-----NAS Class $cls-----"
	for kernel in "${NAS_KERNELS[@]}"; do
		exe="$NAS_BIN_DIR/${kernel}.${cls}.x"
		name="nas_${kernel}_${cls}"
		echo "Running ${kernel^^} (Class $cls) under perf..."

		run_perf "$name" env OMP_NUM_THREADS="$THREADS" OMP_PROC_BIND=close OMP_PLACES=cores \
...
(
	...
	run_perf cloverleaf env OMP_NUM_THREADS="$THREADS" OMP_PROC_BIND=close OMP_PLACES=cores \
...
echo "-----miniBUDE (bm1)-----"
run_perf minibude env OMP_NUM_THREADS="$THREADS" OMP_PROC_BIND=close OMP_PLACES=cores \
	taskset -c "$CPU_RANGE" "$BUDE_EXE" -i 10 -p 32 --deck "$BUDE_DECK"
...
(
	cd "$NEUTRAL_DIR"
	run_perf neutral env OMP_NUM_THREADS="$THREADS" OMP_PROC_BIND=close OMP_PLACES=cores \
...
(
	...
	run_perf tealeaf env OMP_NUM_THREADS="$THREADS" OMP_PROC_BIND=close OMP_PLACES=cores \
...
(
	cd "$STREAM_DIR"
	run_perf stream_gcc15 env OMP_NUM_THREADS="$THREADS" OMP_PROC_BIND=spread OMP_PLACES=cores \
