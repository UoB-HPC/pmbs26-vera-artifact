...
class OpenfoamSpackCheck(rfm.RegressionTest):
    ...
    def prepare_test(self):
        ...
        #todo: hardcoded Slurm/value for number of domains.

        self.prerun_cmds = [
            f'git clone https://develop.openfoam.com/committees/hpc.git',
            f'cd {self.__subdir}',
            f'sed -i "s/numberOfSubdomains.*/numberOfSubdomains $MAX_CPUS;/g" system/decomposeParDict',
            ...
            f'curl -L -o system/fvSolution "https://gitlab.com/openfoam/core/openfoam/-/raw/master/tutorials/incompressible/simpleFoam/motorBike/system/fvSolution?ref_type=heads"',
            f'chmod +x All*',
    ...
    def set_job_size(self):
        ...
        self.num_tasks_per_node = proc.num_cores_per_socket
        self.num_tasks_per_socket = self.num_tasks_per_node
        if self.num_threads:
            self.env_vars['OMP_NUM_THREADS'] = self.num_threads

        self.num_tasks = self.num_tasks_per_node * self.num_nodes
        self.env_vars['MAX_CPUS'] = self.num_tasks
