...
class WrfSpackCheck(rfm.RegressionTest):
    ...
    def prepare_test(self):
        ...
        self.prerun_cmds.append([
            ...
            f'tar xvf ${{HOME}}/sources/wrf/{self.__bench}.tar.gz -C ${{WRF_INP}} --strip-components 1',
            'cd ${WRF_RUN}',
            'ln -sfn ${WRF_HOME}/run/* .',
    ...
    def set_job_size(self):
        ...
        self.num_tasks_per_socket = proc.num_cores_per_socket
        if self.num_threads:
            self.num_tasks_per_node = (proc.num_cores_per_socket)
        ...
        self.num_tasks = self.num_tasks_per_socket * self.num_nodes
