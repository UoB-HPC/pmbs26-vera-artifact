...
class NamdSpackBuild(SpackCompileOnlyBase):
    sourcefile = os.path.join(os.getenv('HOME'),'sources/NAMD_3.0.1_Source.tar.gz')
    defspec = 'namd@3.0.1'
...
class NamdSpackCheck(rfm.RegressionTest):
    ...
    def set_job_size(self):
        ...
        self.executable_opts += [f'+p{proc.num_cores_per_socket*self.num_threads} +setcpuaffinity +maffinity +CmiSleepOnIdle {self.__bench}.namd > output.txt']
